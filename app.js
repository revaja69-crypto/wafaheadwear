...
const App = () => {
    ...
    useEffect(() => {
        ...
        if (filtered.length > 0 && !filtered.some(f => f.name === formData.category)) {
    ...
    const AnalysisView = () => {
        ...
        return (
            <div className="space-y-6">
                <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 card">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="pt-6 border-t border-slate-50 flex items-center justify-between">
                            <div>
                                ...
                                <p className={`text-xl font-black ${margin >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>{margin}%</p>
                            </div>
        ...
        <div className="space-y-8 fade-in">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-8 bg-blue-600 text-white rounded-[32px] shadow-xl shadow-blue-100">
                    ...
                    <h3 className="text-3xl font-black">{formatIDR(stats.balance)}</h3>
                </div>
                <div className="p-8 bg-white border border-slate-100 rounded-[32px] shadow-sm flex items-center justify-between">
                    ...
                    <div className="p-4 bg-emerald-50 text-emerald-600 rounded-2xl no-print"><Icon name="income" /></div>
                </div>
                <div className="p-8 bg-white border border-slate-100 rounded-[32px] shadow-sm flex items-center justify-between">
                    ...
                    <div className="p-4 bg-rose-50 text-rose-600 rounded-2xl no-print"><Icon name="expense" /></div>
                </div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <AnalysisView />
                <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 card">
                    <h3 className="font-black text-xl mb-8">Informasi Bisnis</h3>
                    <div className="p-6 bg-slate-900 text-white rounded-[32px] space-y-4">
                        <p className="text-sm opacity-60">Catat setiap transaksi secara disiplin untuk mendapatkan laporan margin laba yang akurat setiap bulan.</p>
                        <button onClick={() => setShowAddModal(true)} className="w-full py-4 bg-blue-500 rounded-2xl font-black hover:bg-blue-400 transition-all">CATAT TRANSAKSI</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="min-h-screen flex flex-col md:flex-row">
            {/* SIDEBAR / NAV */}
            <aside className="w-full md:w-72 bg-white border-r border-slate-200 p-6 flex flex-col gap-8 no-print shrink-0">
                ...
                <nav className="flex flex-row md:flex-col gap-2 overflow-x-auto no-scrollbar md:flex-1">
                    {[
                        ...
                        <button 
                            ...
                            className={`flex items-center gap-3 px-5 py-4 rounded-2xl font-bold transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'text-slate-400 hover:bg-slate-50'}`}
                        >
                            <Icon name={tab.icon} /> {tab.label}
                        </button>
                    ))}
                </nav>
                <button onClick={() => window.print()} className="hidden md:flex items-center justify-center gap-3 px-6 py-4 rounded-2xl bg-slate-900 text-white font-black text-sm"><Icon name="printer" /> CETAK PDF</button>
            </aside>
            ...
            <main className="flex-1 p-6 md:p-12 overflow-y-auto max-h-screen">
                <header className="flex justify-between items-center mb-10 no-print">
                    ...
                    <button onClick={() => setShowAddModal(true)} className="bg-blue-600 text-white px-6 py-3 rounded-2xl font-black flex items-center gap-2 shadow-lg active:scale-95 transition-all"><Icon name="plus" /> BARU</button>
                </header>
                        ...
                        <div className="space-y-1">
                            {transactions.map(t => (
                                <div key={t.id} className="flex items-center justify-between p-4 border-b border-slate-50 group hover:bg-slate-50 transition-all rounded-2xl">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <p className={`font-black text-sm ${t.type === 'income' ? 'text-emerald-600' : 'text-rose-600'}`}>{t.type === 'income' ? '+' : '-'} {formatIDR(t.amount)}</p>
                                        <button onClick={() => deleteTransaction(t.id)} className="opacity-0 group-hover:opacity-100 p-2 text-slate-300 hover:text-rose-500 transition-all"><Icon name="trash" size={16} /></button>
                                    </div>
                                </div>
                            ))}
                ...
                {activeTab === 'reports' && (
                    <div className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm fade-in">
                        <h3 className="text-xl font-black mb-8 text-center">Rekap Per Kategori</h3>
                ...
                {activeTab === 'settings' && (
                    <div className="max-w-2xl mx-auto space-y-8 fade-in">
                        <div className="bg-white p-10 rounded-[40px] border border-slate-100">
                            <h3 className="text-xl font-black mb-6">Kelola Kategori</h3>
                            <form onSubmit={(e) => { e.preventDefault(); if(!newCatName) return; setCategories([...categories, { id: Date.now().toString(), name: newCatName, type: newCatType }]); setNewCatName(''); }} className="flex gap-2 mb-8">
                                <input type="text" placeholder="Kategori baru..." required value={newCatName} onChange={(e) => setNewCatName(e.target.value)} className="flex-1 p-4 bg-slate-50 rounded-xl outline-none font-bold text-sm" />
                                <select value={newCatType} onChange={(e) => setNewCatType(e.target.value)} className="p-4 bg-slate-50 rounded-xl outline-none font-bold text-sm"><option value="expense">Keluar</option><option value="income">Masuk</option></select>
                                <button type="submit" className="p-4 bg-blue-600 text-white rounded-xl"><Icon name="plus" /></button>
                            </form>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {categories.map(c => (
                                    <div key={c.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                                        ...
                                        <button onClick={() => setCategories(categories.filter(x => x.id !== c.id))} className="text-slate-300 hover:text-rose-500"><Icon name="trash" size={14}/></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className="bg-rose-50 p-10 rounded-[40px] border border-rose-100">
                            ...
                            <p className="text-xs text-rose-600 mb-6 font-medium">Hapus seluruh riwayat transaksi. Data yang dihapus tidak dapat dikembalikan.</p>
                            <button onClick={() => setShowDeleteConfirm(true)} className="px-8 py-4 bg-rose-600 text-white rounded-2xl font-black text-sm active:scale-95 transition-all shadow-lg shadow-rose-200">KOSONGKAN DATA</button>
                        </div>
                    </div>
                )}
            </main>
            ...
            {/* MODALS */}
            {showAddModal && (
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-end md:items-center justify-center p-0 md:p-10">
                    <div className="bg-white w-full max-w-xl rounded-t-[40px] md:rounded-[40px] p-8 md:p-12 animate-in slide-in-from-bottom duration-300 max-h-[90vh] overflow-y-auto shadow-2xl">
                        <div className="flex justify-between items-center mb-10"><h2 className="text-2xl font-black">Input Kas</h2><button onClick={() => setShowAddModal(false)} className="p-3 bg-slate-100 rounded-2xl"><Icon name="x" /></button></div>
                        <form onSubmit={handleAddTransaction} className="space-y-8">
                            ...
                            <input type="number" required placeholder="0" className="w-full text-5xl font-black p-8 bg-slate-50 rounded-[40px] outline-none border-2 border-transparent focus:border-blue-500 text-center transition-all" value={formData.amount} onChange={(e) => setFormData({...formData, amount: e.target.value})} />
                            ...
                            <input type="text" required placeholder="Keterangan transaksi..." className="w-full p-6 bg-slate-50 rounded-2xl outline-none font-bold" value={formData.note} onChange={(e) => setFormData({...formData, note: e.target.value})} />
            ...
            {showDeleteConfirm && (
                <div className="fixed inset-0 bg-rose-900/90 backdrop-blur-md z-[100] flex items-center justify-center p-6">
                    <div className="bg-white w-full max-w-sm rounded-[48px] p-10 text-center animate-in zoom-in duration-300">
                        ...
                        <p className="text-sm text-slate-500 mb-8">Seluruh riwayat keuangan Anda akan dikosongkan secara permanen.</p>
                        <div className="space-y-2">
                            <button onClick={deleteAllData} className="w-full py-4 bg-rose-600 text-white rounded-2xl font-black text-sm active:scale-95 transition-all shadow-lg">HAPUS DATA</button>
